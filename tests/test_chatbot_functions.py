...
def test_function_calling():
    ...
    response1 = call_gemini_with_memory("Onde posso descartar pilhas usadas?", "test_user_1")
    print(f"Resposta: {response1}\n")
    
    # Teste 2: Busca por pontos próximos (com coordenadas)
    print("2. Usuário pergunta: 'Quais pontos de coleta ficam perto de mim?' (com localização)")
    ...
    response3 = call_gemini_with_memory("Quantos pontos de coleta vocês têm?", "test_user_3")
    ...
    response4 = call_gemini_with_memory("Que tipos de materiais vocês aceitam?", "test_user_4")
    print(f"Resposta: {response4}\n")
    
    # Teste 5: Pergunta fora do escopo
    print("5. Usuário pergunta: 'Qual é o melhor time de futebol?'")
    response5 = call_gemini_with_memory("Qual é o melhor time de futebol?", "test_user_5")
...
def test_direct_functions():
    ...
    result2 = execute_function("search_collection_points", {
        "lat": -27.5969,
        "lng": -48.5495,
        "radius_km": 2.0
    })
